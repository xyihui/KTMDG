...
def to_multilabel(pre_mask, classes = 7):
    mask = np.zeros((pre_mask.shape[0], pre_mask.shape[1], classes))
    mask[pre_mask == 1] = [0, 1, 0, 0, 0, 0, 0]
    mask[pre_mask == 2] = [0, 0, 1, 0, 0, 0, 0]
    mask[pre_mask == 3] = [0, 0, 0, 1, 0, 0, 0]
    mask[pre_mask == 4] = [0, 0, 0, 0, 1, 0, 0]
    mask[pre_mask == 5] = [0, 0, 0, 0, 0, 1, 0]
    mask[pre_mask == 6] = [0, 0, 0, 0, 0, 0, 1]
    return mask

f = np.random.uniform(low=-0.1, high=0.1)

class f1(object):
    def __call__(self, sample):
        img = sample['image']
        x = np.linspace(0, 1, 1000)
        conditions = [x <= 1/3+f , x > 1/3+f]
        functions = [lambda x: 2 * x, lambda x: x/2+1/2]
        yvals_1 = np.piecewise(x, conditions, functions)
        img1 = np.interp(img, x, yvals_1)
        sample['image'] = img
        sample['dc'] = img1
        return sample
...
class f2(object):
    def __call__(self, sample):
        img = sample['image']
        x = np.linspace(0, 1, 1000)
        conditions = [x <=1/3+f, (1/3+f < x) & (x < 2/3+f), x >=2/3+f]
        functions = [lambda x: x/2, lambda x: 2*x-1/2, lambda x: x/2+1/2]
        yvals_1 = np.piecewise(x, conditions, functions)
        img1 = np.interp(img, x, yvals_1)
        sample['image'] = img
        sample['dc'] = img1
...
class RandomAdjustSharpness:
    ...
    """
    def __init__(self, sharpness_factor, p=0.8):
        super().__init__()
    ...
    def __call__(self, sample):
        ...
        if torch.rand(1).item() < self.p:
            image = torchvision.transforms.transforms.F.adjust_sharpness(image, self.sharpness_factor)
        sample['image'] = image
        return sample
...
class Normalize_tf(object):
    ...
    def __call__(self, sample):
        ...
        # _mask[(__mask > 100) & (__mask < 201)] = 170
        # _mask[(__mask > 50) & (__mask < 100)] = 85
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        map = np.array(sample['label']).astype(np.uint8)
        img = torch.from_numpy(img).float()
        map = torch.from_numpy(map).long()
        sample['image']=img
        sample['label']=map

        if sample['dc'].__class__ is np.ndarray:
            img1 = np.array(sample['dc']).astype(np.float32).transpose((2, 0, 1))
