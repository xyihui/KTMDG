...
class AdaptiveInstanceNorm2d(nn.Module):
    ...
    def forward(self, x):
        ...
        out = F.batch_norm(
            x_reshaped, running_mean, running_var, running_var, self.bias,
...
class Adain(nn.Module):
    ...
    def forward(self, x, m):
        running_mean = torch.mean(x, dim=(2, 3))
        x = x-running_mean.unsqueeze(2).unsqueeze(3) +m.unsqueeze(2).unsqueeze(3)
        x = self.last_conv(x)
...
class Ada_Decoder(nn.Module):
    ...
    def forward(self, anatomy, style):
        ...
        images = F.interpolate(images, size=[512,512], mode='bilinear', align_corners=True)
        return images
...
if __name__ == '__main__':
    images = torch.FloatTensor(3, 128, 128, 128).uniform_(-1, 1)
    codes = torch.FloatTensor(3, 128).uniform_(-1,1)
    model = Ada_Decoder(128,128,3)
    image1 = model(images, codes)
    print(image1.size())